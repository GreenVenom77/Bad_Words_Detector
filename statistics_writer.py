...
import faker
import openpyxl
from time_helper import ChunkInfo, elapsed, generate_statistics_dict
...
def setup_row(chunk_size="",chunk_number="",healthy_number="",unhealthy_number=""
               ,reading="",filtering="",frame_total="")->Mapping[str,str]:
...
class StatisticsWriter:
    def __init__(self, statistics_dictionary: dict) -> None:
        self.__statistics_dictionary = statistics_dictionary

    def statistics_csv(self):
        log_path= f"output/Detailed_Log_{self.__statistics_dictionary["filtering_algorithm"]}.csv"
        with open(
          log_path , "a", newline="") as csv_file:
            fieldnames = [
                "chunk_size",
                "chunk_number",
            ...
            writer.writeheader()
            chunks_info:list[ChunkInfo]=self.__statistics_dictionary["chunks_info"]
            for index,chunk_info in enumerate(chunks_info) :
                writer.writerow(setup_row(
                    self.__statistics_dictionary["chunk_size"],
                    index+1, # type: ignore
            ...
            reading_list=list(map(lambda ci:ci.reading_time,chunks_info))
            filtering_list=list(map(lambda ci:ci.filtering_time,chunks_info))
            frame_time_list=[r+f for r,f in zip(reading_list,filtering_list)]
            healthy_total=sum(map(lambda ci:ci.number_of_healthy,chunks_info))
            unhealthy_total=sum(map(lambda ci:ci.number_of_unhealthy,chunks_info))
    ...
    def start(self) -> None:
        elapsed_time = elapsed(self.__statistics_dictionary["start_time"])
        logging.info(
...
if __name__=='__main__':
    ...
    for _ in range (dict['number_of_chunks']):
        ...
        chunks_info.append(ChunkInfo(reading_time,filtering_time,healthy_number,unhealthy_number))
