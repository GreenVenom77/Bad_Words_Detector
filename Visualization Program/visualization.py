...
class ExcelPlotter:
    def __init__(self, excel_file_path, sheet1_name, sheet2_name):
        ...
        self.col_names = ['avg_Reading Time', 'Total Reading Time', 'Max Reading Time', 'Min Reading Time', 'avg_filtering Time',
             'Total_filtering Time', 'Max_filtering Time', 'Min_filtering Time', 'Avg Frame_processing_Time',
             'Total Frame_processing_Time', 'Max Frame_processing_Time', 'Min Frame_processing_Time']
        self.sns_style = "whitegrid"

    def read_excel_data(self):
        self.sheet1_df = pd.read_excel(self.excel_file_path, sheet_name=self.sheet1_name)
        self.sheet2_df = pd.read_excel(self.excel_file_path, sheet_name=self.sheet2_name)
        self.concatenated_df = pd.concat([self.sheet1_df, self.sheet2_df], keys=[self.sheet1_name, self.sheet2_name])
    ...
    def plot_and_embed(self):
        ...
        for i, col in enumerate(self.col_names, start=1):
            ...
            sns.barplot(x='D.frame size', y=col, hue=self.concatenated_df.index.get_level_values(0),
